...
use nom::branch::alt;
use nom::bytes::complete::{tag, tag_no_case, take_while_m_n, take};
...
parse_lo_code!("d3", parse_code_d3, CodeType::LoadOffsetImmediate { immediate });
parse_lo_code!("d4", parse_code_d4, CodeType::IncrementData { operand });
parse_lo_code!("d5", parse_code_d5, CodeType::LoadDataImmediate { immediate });
...
fn parse_full_code(input: &str) -> IResult<&str, Vec<CodeType>> {
...
#[cfg(test)]
mod test {
    use crate::frontend::action_replay_ds::lex::{parse_code_0, parse_full_code};
...
021e0460 ffffffff"##;
    ...
    #[test]
    pub fn parse() {
        let (_, list) = parse_full_code(LIST).unwrap();
        println!("{:x?}", list);
    }
    #[test]
    pub fn parse_e() {
        let (_, list) = parse_full_code(E_TYPE).unwrap();
        println!("{:x?}", list);
    }
